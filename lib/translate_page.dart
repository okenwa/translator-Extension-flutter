...
class _TranslateViewState extends State<TranslateView> {
  ...
  void _translate({String text, from, to}) async {
    try {
      ...
      final response = await http.get(Uri.encodeFull(
          'https://translation.googleapis.com/language/translate/v2?target=ha&key=AIzaSyBbYWuURaAku9M7T6M2tLruJqmGJeQzmyo?from=${from}&to=${to}&text=${text}'));
      if (response.statusCode == 200) {
        ...
        setState(() {
          _currentTranslatedTo = to == "en"
              ? "Ba a yi nasarar shiga Haɗin Intanit ba"
